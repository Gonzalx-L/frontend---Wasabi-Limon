...
import { FiltrosComproComponent } from './filtros-compro/filtros-compro.component';
import { FormsModule } from '@angular/forms';

@Component({
  selector: 'app-comprobante-reporte',
  imports: [CommonModule, FormsModule, FiltrosComproComponent],
  templateUrl: './comprobante-reporte.component.html',
  styleUrl: './comprobante-reporte.component.scss'
})
export class ComprobanteReporteComponent implements AfterViewInit {
  ...
  graficoDaY(year: number, month: number, day: number) {
    ...
    this.reportesService.obtenerComprobantesReporte(year, month, day).subscribe((data) => {
      const datosMezclados = this.mezclarArray(data);
      const labels = datosMezclados.map((item: any) => item.nom_compro);
  ...
  graficoMonth(year: number, month: number) {
    ...
    this.reportesService.obtenerComprobantesReporte(year, month).subscribe((data) => {
      const labels = data.map((item: any) => item.nom_compro);
  ...
  graficoYear(year: number) {
    ...
    this.reportesService.obtenerComprobantesReporte(year).subscribe((data) => {
      const labels = data.map((item: any) => item.nom_compro);
      const valores = data.map((item: any) => item.veces_usado);
